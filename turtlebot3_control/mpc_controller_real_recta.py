...
class MPCControllerReal(Node):
    def __init__(self):
        ...
        self.csv_path = os.path.join(self.results_dir, f'recta_mpc_{self.timestamp}.csv')
        
        # Waypoints para línea recta de 1.75m
        self.waypoints = [
            [0.0, 0.0],
            [1.75, 0.0]
        ]
        ...
        self.max_linear = 0.18          # Velocidad lineal máxima
        self.max_angular = 1.0          # Velocidad angular máxima
        self.waypoint_threshold = 0.05  # Umbral ligeramente mayor para transiciones suaves
    ...
    def _print_summary(self, m):
        self.get_logger().info('='*60)
        self.get_logger().info('MÉTRICAS FINALES - MPC (Línea recta)')
