#!/usr/bin/env python3
"""
simple_pid_controller_compuesta.py
PID con LOOKAHEAD CONTINUO → Curva ULTRA SUAVE
Radio 1.75 m | Sin intermitencia | Llega al final
"""
...
class PIDControllerCompuesta(Node):
    def __init__(self):
        super().__init__('pid_controller_compuesta')
        ...
        # --- Lookahead ---
        self.lookahead_base = 0.32
        self.lookahead_min = 0.18
        ...
        self.csv_path = os.path.join(self.base_dir, f'compuesta_pid_{stamp}.csv')
        ...
        self.get_logger().info('PID ULTRA SUAVE - Lookahead continuo')
    ...
    def try_init(self):
        if not self.initialized and self.current_pose is not None:
            ...
            self.get_logger().info('PID inicializado')
    ...
    def control_loop(self):
        ...
        dist_to_end = math.hypot(final_x - self.current_pose.position.x, final_y - self.current_pose.position.y)
        if dist_to_end < 0.03:  # ¡3cm!
        ...
        gx, gy = target
        v_cmd, w_cmd = self.pid_step(gx, gy)
        ...
        # --- Actualizar índice ---
        self.current_idx = self._find_closest_index(self.current_pose.position.x, self.current_pose.position.y, self.ref_path)


    def _adaptive_lookahead(self):
        curv = self._estimate_curvature(self.current_pose.position.x, self.current_pose.position.y)
        self.curvature_buffer.append(curv)
        if len(self.curvature_buffer) > self.buffer_size:
            self.curvature_buffer.pop(0)
        avg_curv = np.mean(self.curvature_buffer) if self.curvature_buffer else 0.0
        return np.clip(self.lookahead_base - 0.14 * avg_curv, self.lookahead_min, self.lookahead_base)


    def _find_lookahead_point(self, dist):
        idx = max(self.current_idx, 0)
        accum = 0.0
    ...
    def pid_step(self, gx, gy):
        ...
        # --- PID ---
        self.integral += alpha * self.dt
        self.integral = np.clip(self.integral, -0.6, 0.6)
        derivative = (alpha - self.prev_error) / self.dt
        self.prev_error = alpha

        w = self.kp * alpha + self.ki * self.integral + self.kd * derivative
        w = np.clip(w, -self.max_angular, self.max_angular)

        # --- Velocidad ---
        rho = math.hypot(dx, dy)
        v_base = min(self.max_linear, self.kp * rho * 0.8)
        if abs(alpha) > 0.2:
    ...
    def _print_summary(self, t, d, ideal, em, ex):
        ...
        self.get_logger().info('PID SUAVE - MÉTRICAS')
