...
Controlador PID para TurtleBot3 Waffle Real - Línea recta de 1.75m
"""
...
class SimplePIDController(Node):
    def __init__(self):
        ...
        self.waypoints = [[0.0, 0.0], [L, 0.0]]  # Línea recta de 1.75m
        ...
        self.csv_path  = os.path.join(self.base_dir, f'recta_PID_{stamp}.csv')
        ...
        self.get_logger().info('CONTROLADOR PID - ROBOT REAL WAFFLE')
        self.get_logger().info(f'Línea recta: 1.75m')
    ...
    def control_loop(self):
        if not self.started:
            ...
            self.get_logger().info('Iniciando seguimiento de la línea recta (PID)')
    ...
    def _print_summary(self, m):
        self.get_logger().info('='*60)
        self.get_logger().info('MÉTRICAS FINALES - PID (Línea recta)')
